...
//! # Cargo features
//!
//! - **system-tray**: Enables system-tray-relative positions.  Note: This
//!   requires attaching the Tauri plugin, *even* when using the trait extension
//!   only.
...
pub fn on_tray_event<R:Runtime>(app:&AppHandle<R>, event:&SystemTrayEvent) {
	match event {
		SystemTrayEvent::LeftClick { position, size, .. } => {
			app.state::<Tray>().0.lock().unwrap().replace((*position, *size));
		},
